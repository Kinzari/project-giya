        ...
        postElement.innerHTML = `
            <div class="post-header d-flex justify-content-between align-items-start p-3">
                <div class="d-flex align-items-center gap-3">
                    ...
                    <div>
                        <h5 class="mb-1 student-name fw-bold">${post.student_name}</h5>
                        <small class="text-muted">${post.school_id}</small>
                    </div>
                </div>
...
async function showUserDetails(userId) {
    try {
        const response = await axios.get(`${sessionStorage.getItem('baseURL')}?action=get_user_details&user_id=${userId}`);
        ...
        if (response.data.success) {
            ...
            document.getElementById('detail-yearLevel').textContent = getYearLevelText(user.user_schoolyearId) || '-';
...
async function submitReply(postId, message) {
    try {
        const adminId = localStorage.getItem('user_id'); // Get logged-in admin's ID
        ...
        if (response.data.success) {
            ...
            await loadPosts(); // Refresh posts to show new reply
        } else {
    ...
    try {
        ...
        if (result.isConfirmed) {
            ...
            if (response.data.success) {
                ...
                await loadPosts(); // Refresh the table
            } else {
